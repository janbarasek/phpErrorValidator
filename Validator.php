...
function fatal_handler() {
	...
	if ($error['type'] === E_ERROR) {
		echo '<div style="z-index: 100000000; position: absolute; left: 0; top: 0; width: 100%; height: 100%; background: white; border-bottom: 3px solid #F44336; padding: 64px 0;">';
			...
			echo '</div>';
			// echo file_get_contents('http://validator.baraja.cz/php/');
		echo '</div>';
		trigger_error('Fatal');
	}
}
...
class Validator
{
	...
	public static function smartMessage($message) {
		...
		$undefinedVariable = explode('undefined-variable:-', $messageIndex);
			if (isset($undefinedVariable[1])) {
				return [
					'special' => 'checkVariables',
					'specialData' => $undefinedVariable[1],
	...
	public static function balancer($errno, $errstr, $errfile, $errline) {
		...
		switch ($errno) {
			case E_USER_ERROR: // 256
				self::prerender($errno, 'Uživatelská chyba na řádku '.$errline.' | Kód chyby: '. $errno, $message, $errline, $errfile);
				break;

			case E_USER_WARNING: // 512
				self::prerender($errno, 'Uživatelské varování na řádku '.$errline.' | Kód chyby: '. $errno, $message, $errline, $errfile);
				break;

			case E_USER_NOTICE: // 1024
				self::prerender($errno, 'Uživatelská poznámka na řádku '.$errline.' | Kód chyby: '. $errno, $message, $errline, $errfile);
				break;

			case E_WARNING: // 2
				self::prerender($errno, 'Varování na řádku '.$errline.' | Kód chyby: '. $errno, $message, $errline, $errfile);
				break;

			case E_NOTICE: // 8
				self::prerender($errno, 'Poznámka na řádku '.$errline.' | Kód chyby: '. $errno, $message, $errline, $errfile);
				break;

			default:
				self::prerender($errno, 'Potenciální problém na řádku '.$errline.' | Kód chyby: '. $errno, $message, $errline, $errfile);
				break;
		}
	...
	private static function renderSource($code, $errorLine = NULL, $smartLine = NULL, $smartString = NULL) {
		...
		$code = str_replace(
			[' ', "\t", '\'', '(', ')', '{', '}'],
			['&nbsp;', '&nbsp;&nbsp;&nbsp;&nbsp;', '<span style="color:#4CAF50">\'</span>', '<span style="color:red">(</span>', '<span style="color:red">)</span>', '<span style="color:red">{</span>', '<span style="color:red">}</span>'],
			$code
		);
		...
		for ($i=0; isset($variablesBuffer[$i]); $i++) {
			$code = str_replace($variablesBuffer[$i], '<span style="color:#3F51B5">'. $variablesBuffer[$i] .'</span>', $code);
		}
		...
		for ($i=0; isset($parserLine[$i]); $i++) {
			...
			if ($errorLine-$line <= 10 && $line-$errorLine <= ($errorLine < 8 ? 8 : 4)) $buffer .= '<div class="line"'. ($style ? ' style="'.$style.'"' : '') .'>'. $line. ': '. ($styled ? strip_tags($parserLine[$i]) : $parserLine[$i]) .'</div>';
		}
		return $buffer;
	}
	...
	private static function template($header, $text, $configurator, $code = []) {
		...
		echo '<div class="bios" id="validatorCore">';
			echo '<div class="error_header">'. ($text['header'] ? $text['header'] : '') . ($text['header'] && $text['original'] ? ' | ' : '') . ($text['original'] ? $text['original'] : '') . ($text['header'] || $text['original'] ? '<br>' : '') . $header .'</div>';
			echo '<div class="error_buttons">';
				echo '<a href="https://www.google.com/search?q='. urlencode('PHP error '. $text['original']) .'"><button>Googlovat chybu</button></a>';
